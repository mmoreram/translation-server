...
namespace Mmoreram\TranslationServer\Compiler;
...
class Compiler
{
    ...
    {
        $pharFilePath = dirname(__FILE__).'/../../../build/translation-server.phar';
         ...
         */
        $phar = new Phar($pharFilePath, 0, 'translation-server.phar');
    ...
    {
        $content = file_get_contents(__DIR__.'/../../../bin/translation-server');
        $content = preg_replace('{^#!/usr/bin/env php\s*}', '', $content);
        $phar->addFromString('bin/translation-server', $content);
...
Phar::mapPhar('translation-server.phar');

require 'phar://translation-server.phar/bin/translation-server';
...
EOF;
    ...
    private function loadVersion()
    {
        ...
        if ($process->run() != 0) {
            throw new \RuntimeException('Can\'t run git log. You must ensure to run compile from translation-server git repository clone and that git binary is available.');
        }
